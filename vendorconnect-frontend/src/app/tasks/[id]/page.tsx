...
export default function TaskDetailPage() {
  ...
  const isTaskPastDue = () => {
    if (!task || !task.end_date || !task.close_deadline) return false;
    const deadline = new Date(task.end_date);
    const now = new Date();
    return now > deadline;
  };
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      setDeliverableFiles(filesArray);
    }
  };
  ...
  const handleChecklistToggle = async (index: number, completed: boolean) => {
    ...
    try {
      await apiClient.post(`/tasks/${task.id}/checklist-status`, {
        item_index: index,
        completed: completed,
      });
  ...
  const handleDeleteComment = async (commentId: number) => {
    if (!task || !confirm('Are you sure you want to delete this comment?')) {
      return;
    }
                ...
                {deliverables.length > 0 ? (
                          ...
                          <div className="flex-1">
                            ...
                            <div className="flex items-center gap-4 mt-2 text-xs text-muted-foreground">
                              ...
                              {deliverable.media && deliverable.media.length > 0 && (
                                <span>{deliverable.media.length} file(s)</span>
                              )}
                            </div>
                          </div>
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      {isTaskPastDue() ? 'Cannot add deliverables to a task past its deadline' : 'Add your first deliverable'}
                    </p>
                  </div>
                )}
              </CardContent>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-bold">Add Deliverable</h2>
              <Button
                variant="ghost"
                size="sm"
                onClick={() => {
